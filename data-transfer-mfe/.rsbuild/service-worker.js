/*
 * Service Worker for data transfer
 * Content-Type: application/javascript
 * This service worker needs to work in both same-origin and cross-origin contexts
 */
...
self.addEventListener("message", (event) => {
  ...
  switch (type) {
    ...
    case "TRANSFER_CHUNK":
      ...
      if (event.ports && event.ports[0]) {
        event.ports[0].postMessage({ type: "CHUNK_RECEIVED" });
      }
  ...
  try {
    ...
    const response = await fetch(targetUrl, {
      ...
      headers: {
        ...
        Origin: "http://localhost:2000", // Try setting origin explicitly
      },
